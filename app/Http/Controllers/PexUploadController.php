...
class PexUploadController extends Controller
{
    ...
    {
        return view('uploader.pex-uploader');
    }
        ...
        if ($request->hasFile('pex_file')) {
            ...
            if (isset($type[$extension])) {
                ...
                if ($extension == 'webp') {
                    ...
                    // Load the dirty svg
                    $dirtySVG = file_get_contents($request->file('pex_file'));

                    // Pass it to the sanitizer and get it back clean
                    $cleanSVG = $sanitizer->sanitize($dirtySVG);

                    // Load the clean svg
                    file_put_contents($request->file('pex_file'), $cleanSVG);
                }
                ...
                if ($type[$extension] == 'image' && $extension != 'svg') {
                    ...
                    try {
                        ...
                        $img = Image::make($request->file('pex_file')->getRealPath())->encode($extension, 75);
                        $height = $img->height();
                        $width = $img->width();
